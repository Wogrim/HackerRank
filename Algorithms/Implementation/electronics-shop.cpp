// find the most expensive keyboard/drive combo within budget
...
{
    int max = -1;
    // sort costs then find the most expensive drive you can buy with each keyboard
    sort(keyboards.begin(), keyboards.end());
    sort(drives.begin(), drives.end());
    int j = drives.size() - 1;
    // for each keyboard
    for (int i = 0; i < keyboards.size() && keyboards[i] < b; i++)
    {
        // find best drive you can afford with this keyboard
        while (j >= 0 && keyboards[i] + drives[j] > b)
            j--;
        // we are done if there is no affordable combination, because further keyboards cost even more
        if (j < 0)
            break;
        // else we have a combo
        if (keyboards[i] + drives[j] > max)
        {
            max = keyboards[i] + drives[j];
            // we are done if it's exactly budget
            if (max == b)
                return b;
        }
    }
