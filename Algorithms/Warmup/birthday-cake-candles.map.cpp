// count occurences of the largest number
...
{
    // counts in an ordered map lets you easily get the max
    // although not terribly efficient
    map<int, int> candleheightcounts;
    for (int h : candles)
        candleheightcounts[h]++;
    // rbegin() points to the largest key/value pair
    return candleheightcounts.rbegin()->second;
}
